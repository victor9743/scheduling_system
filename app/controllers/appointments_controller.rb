class AppointmentsController < ApplicationController
  ...
  def index
    appointments = current_user.appointments.includes(:professional)
  ...
  def create
    ...
    if check_conflict(appointment.professional_id, new_start, new_end)
      render json: { error: "Schedule unavailable" }, status: :unprocessable_entity
    elsif check_business_hours(new_start, new_end)
  ...
  def set_appointment
    @appointment = Appointment.find(params[:id]).merge(user: current_user)
  end
  ...
  def check_conflict(professional, new_start, new_end)
      return Appointment.where(professional_id: professional)
